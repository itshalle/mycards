...
BLOG_CATEGORIES = {
    'emotions-self-awareness': {
        ...
        'intro_paragraphs': [
            ...
            'مقاله‌های این دسته کمک می‌کنند بدون قضاوت یا تشخیص‌گذاری عجولانه، کمی دقیق‌تر به واکنش‌های بدنی، فکرها و احساساتت نگاه کنی. از روش استفاده از چرخ احساسات تا موقعیت‌هایی که نمی‌دانی دقیقاً چه حسی داری، هر مطلب با مثال‌های روزمره و پیشنهادهای قابل‌استفاده نوشته شده است. هدف این نیست که برای هر تجربه فوراً یک برچسب پیدا کنیم؛ هدف این است که بهتر بفهمیم چه اتفاقی درونمان می‌گذرد و چطور می‌توانیم با خودمان روشن‌تر و مهربان‌تر روبه‌رو شویم.'
        ],
    ...
    'calm-mindfulness': {
        ...
        'intro_paragraphs': [
            ...
            'در این دسته تمرین‌های کوتاه آرامش، ذهن‌آگاهی در زندگی روزمره و مهارت‌های اصلی DBT را به زبان روشن مرور می‌کنیم. بعضی مقاله‌ها برای یک مکث پنج‌دقیقه‌ای در روزهای شلوغ‌اند و بعضی توضیح می‌دهند ذهن‌آگاهی چیست، چه تفاوتی با بی‌فکرشدن دارد و مهارت‌های DBT کجا به کار می‌آیند. این مطالب جای درمان یا کمک تخصصی را نمی‌گیرند؛ هدفشان آشنایی با تمرین‌هایی است که می‌توانند توجه به لحظه، شناخت واکنش‌ها و مراقبت روزانه از خود را کمی قابل‌دسترس‌تر کنند.'
        ],
    ...
    'relationships-conversation': {
        ...
        'intro_paragraphs': [
            ...
            'مقاله‌های این دسته موضوع‌هایی مانند سؤال برای شناخت بیشتر پارتنر، گفت‌وگو درباره کودکی، گوش‌دادن بدون نصیحت و موقعیت‌هایی را بررسی می‌کنند که احساس می‌کنید دیگر حرف تازه‌ای برای گفتن ندارید. پیشنهادها طوری نوشته شده‌اند که گفت‌وگو به بازجویی، آزمون رابطه یا تمرینی مصنوعی تبدیل نشود. هدف این است که دو نفر بتوانند با ریتم خودشان درباره خاطره‌ها، احساسات، نیازها و تجربه‌های روزمره حرف بزنند و فضای بیشتری برای درک متقابل بسازند.'
        ],
    ...
    'quality-time-gifts': {
        ...
        'intro_paragraphs': [
            ...
            'بخشی از مطالب این صفحه نیز به انتخاب هدیه معنادار می‌پردازد؛ هدیه‌ای که از توجه به علایق، نیازها و جزئیات واقعی زندگی طرف مقابل شروع می‌شود، نه فقط قیمت یا ظاهر بسته. محصولات دونفره Only Cards برای استفاده مشترک دو نفر طراحی شده‌اند و می‌توانند به‌عنوان هدیه برای یک زوج انتخاب شوند، نه محصولی که فقط یک نفر دریافت کند و کنار بگذارد. محور اصلی این دسته ساختن تجربه، گفت‌وگو و زمانی است که بعد از بازشدن بسته هم ادامه پیدا می‌کند.'
        ],
...
def index():
    ...
    return render_template(
        ...
        og_image=absolute_static_url('images/social/onlycards-home-preview-20260921.webp'),
...
def contact():
    return render_template(
        'contact.html',
        meta_title='ارتباط با ما | Only Cards',
        meta_description='راه ارتباط با Only Cards برای پرسش درباره محصولات، سفارش، ارسال و بسته‌بندی.',
        canonical_url=absolute_url(url_for('contact'))
    )
...
def sitemap():
    pages = [
        ...
        {
            'loc': absolute_url(url_for('contact')),
            'priority': '0.6',
            'changefreq': 'monthly'
        },
        {
